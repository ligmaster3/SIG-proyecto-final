...
function getAsistenciaHoy($id_estudiante)
{
    global $conn;
    try {
...
function getUsoComputadorasHoy($id_estudiante)
{
    global $conn;
    try {
        $fecha_actual = date('Y-m-d');
...
function getEstadisticasFacultad($id_facultad)
{
    global $conn;
    try {
        $stmt = $conn->prepare("
...
function getEstadisticasEscuela($id_escuela)
{
    global $conn;
    try {
        $stmt = $conn->prepare("
...
try {
    ...
    // Revisar y mostrar facultad/escuela, si no existen mostrar "No asignada"
    foreach ($solicitudes as &$solicitud) {
        if (empty($solicitud['facultad'])) {
            $solicitud['facultad'] = 'No asignada';
        }
        if (empty($solicitud['escuela'])) {
            $solicitud['escuela'] = 'No asignada';
        }
    }
    unset($solicitud);
...
try {
    ...
    // Revisar y mostrar facultad/escuela, si no existen mostrar "No asignada"
    foreach ($en_biblioteca as &$est) {
        if (empty($est['facultad'])) {
            $est['facultad'] = 'No asignada';
        }
        if (empty($est['escuela'])) {
            $est['escuela'] = 'No asignada';
        }
    }
    unset($est);
...
try {
    ...
    // Revisar y mostrar facultad/escuela, si no existen mostrar "No asignada"
    foreach ($en_computadoras as &$est) {
        if (empty($est['facultad'])) {
            $est['facultad'] = 'No asignada';
        }
        if (empty($est['escuela'])) {
